    ...
    return (
        <div className="shadow-md w-full fixed top-0 left-0">
            <div className="md:flex items-center justify-between bg-gradient-to-r from-emerald-50 to-blue-50 py-4 md:px-10 px-7">
                ...
                {/* link items */}
                <ul
                    className={`md:flex md:items-center md:pb-0 pb-12 absolute md:static bg-transparent md:z-auto z-[-1] left-0 w-full md:w-auto transition-all duration-500 ease-in ${
                        open ? 'top-20 flex flex-col justify-center items-center bg-white' : 'top-[-490px]'
                    }`}
                >
                    {Links.map((link, index) => (
                        <li key={index} className="md:ml-8 md:my-0 my-3 font-bold active:underline">
                            <Link to={link.link} className="text-gray-800 font-semibold hover:text-gray-700 duration-500">
                                {link.name}
                            </Link>
                        </li>
                    ))}
                    <button
                        ...
                        className="btn bg-yellow-500 hover:bg-yellow-400 text-white md:ml-8 font-semibold px-3 py-1 rounded duration-500 md:static"
                    >
                        Add Recipe
                    </button>
                </ul>
